...
uint64_t DES(uint64_t input, uint64_t key, char mode) {
    ...
    for (i = 0; i < 64; i++) {
        init_perm_res <<= 1;
        printf("init_perm_res %ld \n",init_perm_res);
        init_perm_res |= (input >> (64-IP[i])) & LB32_MASK; //LB32_MASK
        printf("init_perm_res*** %ld \n",init_perm_res);
        if(i==2)
            return 0;
    }

    G = (uint32_t) (init_perm_res >> 32) & L64_MASK;
    D = (uint32_t) init_perm_res & L64_MASK;

///Inicio Geração das chaves
    for (i = 0; i < 56; i++) {
        permuted_choice_1 <<= 1;
        permuted_choice_1 |= (key >> (64-PC1[i])) & LB64_MASK;
    }

    L = (uint32_t) ((permuted_choice_1 >> 28) & 0x000000000fffffff);
    R = (uint32_t) (permuted_choice_1 & 0x000000000fffffff);

    /* Calculo das 16 chaves */
    for (i = 0; i< 16; i++) {
        /* key schedule */
        // shifting Ci and Di
        ...
        permuted_choice_2 = 0;
        permuted_choice_2 = (((uint64_t) L) << 28) | (uint64_t) R ;
...
/// FIM Geração das chaves

/// Round 16
    for (i = 0; i < 16; i++) {
        s_input = 0;
        /* f(R,k) Funçao de expansão */
        for (j = 0; j< 48; j++) {
            s_input <<= 1;
            s_input |= (uint64_t) ((D >> (32-E[j])) & LB32_MASK);
        }//Fim f(R,k)

        /*
         * Encryption/Decryption
         * XORing expanded Di with Ki
         */
        if (mode == 'd') // decryption
            s_input = s_input ^ sub_key[15-i];
        else // encryption
            s_input = s_input ^ sub_key[i]; //OU exclusivo com a chave
    ...
    pre_output = (((uint64_t) D) << 32) | (uint64_t) G;
